...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<?> handleAllExceptions(Exception ex, HttpServletRequest request) {
		...
		log.error("Error Interno: " + ex.getMessage() + " - " + request.getRequestURL().toString());
	...
	public ResponseEntity<?> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, 
		...
		error.setDescription("Error en la petición enviada " + ex.getAllErrors().stream().map(each -> each.getDefaultMessage()));
		
		log.error("Error en la petición enviada: " + error.getDescription() + " - " + request.getRequestURL().toString());
